...
int findHash(long_int a_prime, long_int b_prime, int p_prime, int table_size, int key)
{
...
public:
    ...
    void Construct(list<int> &input)
    {
        ...
        while (flag)
        {
            ...
            {
                int hashKey = findHash(hash_a, hash_b, prime, size, *elem);
                if (hashKey < 0)
                    hashKey = -hashKey;
...
public:
    ...
    void Initialize(const vector<int> &numbers)
    {
        ...
        {
            int hashKey = findHash(_hashFuncA, _hashFuncB, _primeNumber,
